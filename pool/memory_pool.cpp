...
namespace engine {
	...
	void* memory_c_pool::AllocMemory(size_t size)
	{
		...
		if (MemoryConfig::MAX_SIZE < size) {
			...
			unsigned char index = real_size / MemoryConfig::ALIGN - 1;
			...
			if (p) {
				...
				if (heap_size >= real_size) {
					void* p = free_start;
					free_start = static_cast<char*>(free_start) + real_size;
					heap_size -= real_size;
					return p;
				}
	...
	void* memory_c_pool::FillUpMemory(int index, size_t size)
	{
		...
		if (heap_size != 0) {
			unsigned char heap_index = RoundDown(heap_size) / MemoryConfig::ALIGN - 1;
		...
		// 将分配的内存放到链表中去
		void* ptr = static_cast<char*>(p->p_memory) + size;
		for (int i = 0; i < 10; i++) {
		...
		// 补充内存池
		free_start = ptr;
		free_end = static_cast<char*>(p->p_memory) + (size * 21);
		heap_size = size * 10;
