...
namespace engine {
	...
	typedef struct larger_memory_c {
		...
		~larger_memory_c() {
			if (p_memory) {
				delete[] static_cast<char*>(p_memory);
				p_memory = nullptr;
			}
		}
	...
	typedef struct memory_manage_c {
		p_larger_memory new_memory;						//	一级内存分配器
		...
		~memory_manage_c() {
			...
			for (auto p_larger = larger.begin(); p_larger != larger.end(); p_larger++) {
				delete[] static_cast<char*>(*p_larger);
			}
		}
	...
	private:
		...
		p_obj free_list[MemoryConfig::FREE_LIST_NUMS];			//	以8为倍数的小块内存链表

		p_memory_manage manage;					//	内存管理器
		memory_c_pool() :manage(new m_memory_manage) {}

		//	向上取整
		size_t RoundUp(size_t size) {
			size_t _ALIGN = MemoryConfig::ALIGN;
